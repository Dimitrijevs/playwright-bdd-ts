...
Then('The skills list shows: {string}', async ({page}, expectedSkills: string) => {

    const expectedSkillsArray = expectedSkills.split(",").map((skill) => skill.trim());
  
    const skillsListLocator = page.locator('div.MuiBox-root.css-e5z02e .MuiChip-label');
...
When('User removes the skill {string}', async ({page}, removedSkill: string) => {
  
    const removedSkillLocator = page.locator('div.MuiBox-root.css-e5z02e > div').filter({ hasText: removedSkill });

    await removedSkillLocator.locator('svg.MuiChip-deleteIcon').click();
});

Then('The skill {string} is no longer displayed', async ({page}, removedSkill: string) => {

    const removedSkillLocator = page.locator('div.MuiBox-root.css-e5z02e').filter({ hasText: removedSkill });
