Feature: Social Media Feed Interaction
  ...
  Scenario: Verify social media feed interactions
    ...
    When User clicks on the "Social Media Feed Interaction Challenge" challenge button
    Then User should see "Social Media Feed Interaction Challenge" title

    Then User verifies that the <postOrderNumber> post contains an profile image, username, post image, like button, like count
    
    Then User likes the post, verifies that the like count changes by 1
    Then User verifies that notification recieved about successful like
    Then User unlikes the post, verifies that the like count changes by 1
